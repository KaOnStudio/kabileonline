...
if ($data[13])
{
 ...
 if (!$c_status[13])
		if ($data[13]<10)
		{
			...
			if (!(($res[0]>=$cost[0]*pow($r, $data[13]))&&($res[1]>=$cost[1]*pow($r, $data[13]))&&($res[2]>=$cost[2]*pow($r, $data[13]))&&($res[3]>=$cost[3]*pow($r, $data[13]))&&($res[4]>=$cost[4]*pow($r, $data[13])))) label("</br>".$lang['noResources']);
			echo "</br>------------------------------------------</br>";
		}
