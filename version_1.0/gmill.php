...
if ($data[0])
{
 ...
 echo $buildings[0][8]."</br></br>";
 if (!$c_status[0])
  for ($i=0; $i<count($land); $i++)
	 if ($land[$i]<10)
	 {
	  $j=$i+1;
	  $name=explode("-", $buildings[0][2]); $dur=explode("-", $buildings[0][6]); $upk=explode("-", $buildings[0][7]); $cost=explode("-", $buildings[0][4]); $dur[$land[$i]]=explode(":", $dur[$land[$i]]);
