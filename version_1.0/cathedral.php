...
if (isset($_SESSION["user"][0], $_GET["town"]))
{
 ...
 $data=explode("-", $town[8]); $res=explode("-", $town[10]); $prod=explode("-", $town[9]); $lim=explode("-", $town[11]); $out=explode("-", $buildings[11][5]);
}
...
<head>
<title><?php echo $title." - ".$buildings[11][2]; ?></title>
</head>
...
if ($data[11])
{
 echo $buildings[11][8]."</br></br>";
 if (!$c_status[11])
		if ($data[11]<10)
		{
			$dur=explode("-", $buildings[11][6]); $upk=explode("-", $buildings[11][7]); $cost=explode("-", $buildings[11][4]); $dur[$data[11]]=explode(":", $dur[$data[11]]);
			$tag="<a class='q_link' href='build.php?town=".$town[0]."&b=".$buildings[11][0]."&subB=-1'>".$lang['upgrade']." ".$buildings[11][2]." ".$lang['toLevel']." ".($data[11]+1)."</a>";
			$tag=$tag."</br>".$lang['cost'].": <img src='".$imgs.$fimgs."00.gif' title='".$lang['crop']."'>".floor($cost[0]*pow($r, $data[11]))." <img src='".$imgs.$fimgs."01.gif' title='".$lang['lumber']."'>".floor($cost[1]*pow($r, $data[11]))." <img src='".$imgs.$fimgs."02.gif' title='".$lang['stone']."'>".floor($cost[2]*pow($r, $data[11]))." <img src='".$imgs.$fimgs."03.gif' title='".$lang['iron']."'>".floor($cost[3]*pow($r, $data[11]))." <img src='".$imgs.$fimgs."04.gif' title='".$lang['gold']."'>".floor($cost[4]*pow($r, $data[11]))."</br>".$lang['duration'].": ".($dur[$data[11]][0]*$lim[4]/100).":".($dur[$data[11]][1]*$lim[4]/100)."</br>".$lang['upkeep'].": ".$upk[$data[11]]."</br>".$lang['expMorale'].": ".$out[$data[11]];
			echo $tag;
			if ($town[12]+$town[3]+$upk[$data[11]]>$lim[3]) label("</br>".$lang['noHouses']);
			if (!(($res[0]>=$cost[0]*pow($r, $data[11]))&&($res[1]>=$cost[1]*pow($r, $data[11]))&&($res[2]>=$cost[2]*pow($r, $data[11]))&&($res[3]>=$cost[3]*pow($r, $data[11]))&&($res[4]>=$cost[4]*pow($r, $data[11])))) label("</br>".$lang['noResources']);
