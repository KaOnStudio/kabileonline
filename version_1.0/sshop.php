...
<head>
<title><?php echo $title." - ".$buildings[20][2]; ?></title>
</head>
...
if ($data[20])
{
 ...
 echo $buildings[20][8]."</br></br>";
 echo $lang['availTroops'].":<table class='q_table' style='border-collapse: collapse; text-indent: 0; text-align: center' width='600' border='1'>
 <tr>";
 ...
 if (!$c_status[20])
		if ($data[20]<10)
		{
			$dur=explode("-", $buildings[20][6]); $upk=explode("-", $buildings[20][7]); $cost=explode("-", $buildings[20][4]); $dur[$data[20]]=explode(":", $dur[$data[20]]);
			...
			echo $tag;
			if ($town[12]+$town[3]+$upk[$data[20]]>$lim[3]) label("</br>".$lang['noHouses']);
			if (!(($res[0]>=$cost[0]*pow($r, $data[20]))&&($res[1]>=$cost[1]*pow($r, $data[20]))&&($res[2]>=$cost[2]*pow($r, $data[20]))&&($res[3]>=$cost[3]*pow($r, $data[20]))&&($res[4]>=$cost[4]*pow($r, $data[20])))) label("</br>".$lang['noResources']);
			echo "</br>------------------------------------------</br>";
		}
...
for ($i=0; $i<count($uq); $i++) echo "[<a class='q_link' href='cancel_u.php?town=".$_GET["town"]."&type=".$uq[$i][1]."'>x</a>] ".$uq[$i][2]." ".$units[$uq[$i][1]][2]." - <span id='".$i."'>".$uq[$i][0]."</span><script type='text/javascript'> var id=new Array(50); timer('".$i."', 'washop.php?town=".$_GET["town"]."'); </script></br>";
?>
          </p>
    </td>
	</tr>
