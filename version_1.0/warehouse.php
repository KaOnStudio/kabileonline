...
if ($data[5])
{
 ...
 if (!$c_status[5])
		if ($data[5]<10)
		{
			...
			if (!(($res[0]>=$cost[0]*pow($r, $data[5]))&&($res[1]>=$cost[1]*pow($r, $data[5]))&&($res[2]>=$cost[2]*pow($r, $data[5]))&&($res[3]>=$cost[3]*pow($r, $data[5]))&&($res[4]>=$cost[4]*pow($r, $data[5])))) label("</br>".$lang['noResources']);
