...
if ($data[6])
{
 ...
 if (!$c_status[6])
  if ($data[6]<10)
  {
	 	...
	 	if (!(($res[0]>=$cost[0]*pow($r, $data[6]))&&($res[1]>=$cost[1]*pow($r, $data[6]))&&($res[2]>=$cost[2]*pow($r, $data[6]))&&($res[3]>=$cost[3]*pow($r, $data[6]))&&($res[4]>=$cost[4]*pow($r, $data[6])))) label("</br>".$lang['noResources']);
