<?php 
	...
	if (isset($_SESSION["user"][0], $_GET["town"]))
	{
		...
		$data = explode("-", $town[8]); $res=explode("-", $town[10]); $prod=explode("-", $town[9]); $lim=explode("-", $town[11]); $land=explode("/", $town[13]); $land=explode("-", $land[3]); $out=explode("-", $buildings[2][5]);
			...
			if (!$c_status[3])
			{
				...
				for ($i=0; $i<count($land); $i++)
				{
					...
					if ($land[$i]<10)
					{
						$j = $i+1;
						$name=explode("-", $buildings[3][2]); 
						$dur=explode("-", $buildings[3][6]); 
						$upk=explode("-", $buildings[3][7]); 
						$cost=explode("-", $buildings[3][4]); 
