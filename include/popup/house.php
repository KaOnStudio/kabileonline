		...
		if ($data[8])
		{
			if (!$c_status[8])
			{
				if ($data[8]<10)
				{
					...
					$dur=explode("-", $buildings[8][6]); 
					$upk=explode("-", $buildings[8][7]); 
					$cost=explode("-", $buildings[8][4]); 
					$dur[$data[8]]=explode(":", $dur[$data[8]]);
					...
					if ($town[12]+$town[3]+$upk[$data[8]]>$lim[3]) echo("</br>".$lang['noHouses']);
					if (!(($res[0]>=$cost[0]*pow($r, $data[8]))&&($res[1]>=$cost[1]*pow($r, $data[8]))&&($res[2]>=$cost[2]*pow($r, $data[8]))&&($res[3]>=$cost[3]*pow($r, $data[8]))&&($res[4]>=$cost[4]*pow($r, $data[8])))) echo("</br>".$lang['noResources']);
