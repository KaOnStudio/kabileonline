			...
			if (!$c_status[2])
			{
				...
				for ($i=0; $i<count($land); $i++)
				{
					...
					if ($land[$i]<10)
					{
						$j=$i+1;
						$name=explode("-", $buildings[2][2]); 
						$dur=explode("-", $buildings[2][6]); 
						$upk=explode("-", $buildings[2][7]); 
						$cost=explode("-", $buildings[2][4]); 
						$dur[$land[$i]]=explode(":", $dur[$land[$i]]);
						$tag="<a class='q_link' href='build.php?town=".$town[0]."&b=".$buildings[2][0]."&subB=".$i."'>".$j.".".$name[1]."  : ".$lang['upgrade']." ".$lang['toLevel']." ".($land[$i]+1)."</a>";
