<?php 
	...
	if (isset($_SESSION["user"][0], $_GET["town"]))
	{
		...
		$data = explode("-", $town[8]); $res=explode("-", $town[10]); $prod=explode("-", $town[9]); $lim=explode("-", $town[11]); $land=explode("/", $town[13]); $land=explode("-", $land[0]); $out=explode("-", $buildings[0][5]);
		...
		if ($data[0])
		{
			if (!$c_status[0])
			{
				...
				{
					echo "<td "; if(count($land)==4) echo " style='width:330px;' "; else echo " style='width:330px;' "; echo ">";
					if ($land[$i]<10)
					{
						$j=$i+1;
						$name=explode("-", $buildings[0][2]); 
						$dur=explode("-",  $buildings[0][6]); 
						$upk=explode("-",  $buildings[0][7]); 
						$cost=explode("-", $buildings[0][4]); 
						$dur[$land[$i]]=explode(":", $dur[$land[$i]]);
						$tag="<a class='q_link' href='build.php?town=".$town[0]."&b=".$buildings[0][0]."&subB=".$i."'>".$j.".".$name[1]."  : ".$lang['upgrade']." ".$lang['toLevel']." ".($land[$i]+1)."</a>";
						...
						if ($town[12]+$town[3]+$upk[$land[$i]]>$lim[3]) echo("</br>".$lang['noHouses']);
						if (!(($res[0]>=$cost[0]*pow($r, $land[$i]))&&($res[1]>=$cost[1]*pow($r, $land[$i]))&&($res[2]>=$cost[2]*pow($r, $land[$i]))&&($res[3]>=$cost[3]*pow($r, $land[$i]))&&($res[4]>=$cost[4]*pow($r, $land[$i])))) echo("</br>".$lang['noResources']);
					}
