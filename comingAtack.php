<?php 
	...
	if (isset($_SESSION["user"][0], $_GET["town"]))
	{
		...
		if(count($iaq)) {
			...
			echo '<td style="width:160px;">Kaynak</td>';
			...
			{
				$eksisorunu = explode(":", $iaq[$i][0]);
				if($eksisorunu[0][0] == "-") {
					echo "<script> window.location = 'town.php?town=".$town[0]."&hata=catackq'; </script>";
				}
				else
				{
					$twn  = town($iaq[$i][1]);
					...
					if (!$iaq[$i][4]) {
						...
						<td> ".$twn[2]."</td>
						<td>".$what."</td>
						<td><span id='iaq".$i."'>".$iaq[$i][0]."</span></td>
						<td><a class='q_link' href='cancel_c.php?town=".$_GET["town"]."&b=".$cq[$i][1]."&subB=".$cq[$i][2]."'></a></td>
						</tr>";
					}
					else
						echo "<tr style='text-align:center; height:40px;'>
			...
			echo '</table>';
			for ($i=0; $i<count($iaq); $i++)
				echo "<script type='text/javascript'> var id=new Array(50); timer('iaq".$i."', 'town.php?town=".$_GET["town"]."'); </script>";
		}
		else
			echo "</br></br><b>Kabilenize Doğru Hareket Eden Bir Ordu Yok!</b></br></br>";
